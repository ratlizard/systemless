...
impl TrapDispatcher {
    ...
    pub(crate) fn swap_vfs_entry_contents(&mut self, left: &str, right: &str) {
        ...
        if let Some(modified) = right_modified {
            if let Some(entry) = self.vfs_metadata.get_mut(&left) {
                entry.modified_date = modified;
            }
        }
        if let Some(modified) = left_modified {
            if let Some(entry) = self.vfs_metadata.get_mut(&right) {
                entry.modified_date = modified;
            }
        }
        self.process_file_system.publish_classic_vfs_metadata(&left);
        self.process_file_system.publish_classic_vfs_metadata(&right);
    }
